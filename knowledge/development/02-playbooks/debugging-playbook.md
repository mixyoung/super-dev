...
只要求诊断时，收集证据并说明原因及拟修复，不改业务代码。已有修复授权时，在本轮范围内处理根因；授权不包含修改凭据、全局环境、生产数据或绕过权限。命令、路径和工具按实际平台选择，下文分布式工具不是本地小项目的安装前提。

| 观察 | 下一步 | 不能据此宣称 |
| --- | --- | --- |
| 解释器/依赖错误导致测试无法收集 | 核对项目环境、版本、工作目录及真实报错，在权限内恢复可执行环境 | 业务逻辑已失败或通过 |
| 路径能通但返回或权限语义不符 | 对照请求、响应与消费者契约定位第一处偏差 | 只因 HTTP 成功就满足需求 |
| 检查实际执行且断言失败 | 记录最小复现、假设及能区分假设的实验 | 用改预期或删断言代替修复 |
| 同一失败在同一条件下反复出现且无新证据 | 重读原始错误，挑战原假设或检查不同依赖；不是原样再跑 | 命令跑得多就有进展 |

有新证据或真实修复时可继续，不按通用次数机械终止。无法在授权范围内取得必要条件时，说明已验证、仍未知、缺失条件和下一步；不隐瞒阻碍，也不把例行监控的状态未变化当作修复死循环。修复后先重跑原失败检查，再查受影响旧行为。压缩上下文保留错误、失败实验和决定依据，不只留下“继续修复”。

来源与取舍：[UmaDev 吸纳映射](../../../docs/UMADEV_SOFT_CONTENT_ADOPTION.md) U7；不引入自动分类、指纹台账或新阶段。
...
- [ ] 有足以支持当前调查的输出、日志或测试证据；复杂分布式问题按需补指标和追踪
- [ ] 有访问相关系统日志和监控数据的权限
- [ ] 了解系统架构和关键组件的交互关系
