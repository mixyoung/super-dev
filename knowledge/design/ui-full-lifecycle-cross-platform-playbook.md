...
- 动效服务于反馈或已确认的品牌表达，避免妨碍阅读、操作和减少动态效果偏好

---

## 二、组件样式规格（适用参考）

以下组件参数和产品布局是参考基线，使用时服从当前项目已经冻结的 UIUX；不是把所有页面套成统一侧栏、卡片或固定尺寸。只对本次涉及的平台和界面适用，纯 CLI 不新增 UI 工作。

### 异步状态与数据上下文（U13）

按真实查询和提交行为选取状态，不把错误当空数据，也不把已有数据刷新显示成首次加载。只有当前身份、权限和查询上下文允许时才保留旧数据，并说明刷新状态；切换账户/租户或权限撤销时隔离旧缓存，不能为了避免闪屏继续显示无权访问的数据。

- 快速筛选或搜索时，取消过期请求或丢弃过期响应，确保结果属于当前查询；请求取消不代表后端已执行的写入被撤销。
- 保存失败保留可安全保留的输入并指出原因；重复提交保护结合服务端语义。非幂等写入不能无条件自动重试。
- 乐观更新失败时恢复正确状态并告知用户，不留下假成功；缓存恢复要考虑期间发生的其他有效更新。
- 错误给适用的恢复出口，如修正输入、重新认证、重试或返回；不是每种错误都能靠重试解决。
- 动态反馈对键盘和辅助技术可感知，沿用已冻结组件生态。离线缓存、骨架屏、取消功能按需求采用，不把所有状态表变成每页的新功能清单。

模板中的图表和数值须区分演示与真实数据。没有真实数据时明确空态或演示标识，不为“看起来完整”虚构指标；占位 SVG、模拟成功和截图不能证明真实集成已完成。来源及取舍见 [UmaDev 吸纳映射](../../docs/UMADEV_SOFT_CONTENT_ADOPTION.md) U13。
...
### 4.2 交付前检查清单（按本轮界面与项目要求选取）

组件尺寸、断点和动效时长是参考值，使用项目已确认的设计规范。可访问名称优先使用原生语义和可见标签，不要求每个元素额外设置 `aria-label`；依据为 [W3C APG 可访问名称指南](https://www.w3.org/WAI/ARIA/apg/practices/names-and-descriptions/)（2026-09-11 核对）。

**视觉完整性**：
- [ ] 已确认字体体系可用，包含适用回退；原生或系统字体选择有设计依据
- [ ] 配色使用项目定义的 Token，非硬编码 hex
- [ ] 所有 SVG 图标，零 emoji 图标
- [ ] 按钮/输入框尺寸符合项目组件体系，40px 仅作示例
- [ ] 卡片圆角沿用已确认设计变量，不以示例区间替代项目选择
- [ ] 阴影层级正确（卡片 md、弹窗 lg、模态 xl）

**交互完整性**：
- [ ] 可交互元素的反馈符合目标平台惯例，网页链接与按钮可辨认
...
- [ ] 文字对比度 >= 4.5:1（WCAG AA）
- [ ] 图片有 alt 属性
- [ ] 交互元素有可访问名称，优先原生语义和可见标签，必要时补 ARIA
...
3. 必须明确已确认目标平台的适配策略；Web/H5 项目覆盖相应响应式需求，不据此给桌面或 CLI 项目新增端
