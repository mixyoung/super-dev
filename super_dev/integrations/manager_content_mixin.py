...
class IntegrationManagerContentMixin:
    ...
    def _build_file_content(self, target: str, relative: str) -> str:
        ...
        if target in {"cursor", "cursor-cli"}:
            rules_body = (
                ...
                "- For standard UI work, verify frontend runtime and wait for user preview confirmation before backend tasks; non-UI work follows its applicable path, and existing quality gates remain.\n"
                ...
                "- Before writing code, read the actual project dependency manifest. UI libraries, tokens and pages apply only to UI changes. If an API is unclear, read official docs first.\n"
                "- Icons MUST come from Lucide/Heroicons/Tabler. No emoji as functional icons; follow the approved brand and UI scope.\n"
    ...
    def _build_claude_agent_content(self) -> str:
        return (
            ...
            "- Enforce the sequence: research -> three core docs -> wait for confirmation -> Spec/tasks -> applicable frontend runtime verification -> user preview confirmation (standard UI) -> backend/tests/delivery.\n"
            ...
            f"- {readonly_request_guidance('en')}\n"
            "- For new work use its work-mode entry; for resume read SESSION_BRIEF and report the recorded phase without restarting or bypassing pending gates.\n"
    ...
    def _build_slash_command_content(self, target: str) -> str:
        if target == "windsurf":
            return (
                ...
                "5. 本轮涉及 UI 时先实现并运行前端，暂停等待用户预览确认后再进入后端；无 UI 沿用原不适用处理\n\n"
                "## 研究要求\n"
                "- 至少研究 3 到 5 个可借鉴产品\n"
    ...
    def _generic_ide_rules(self, target: str) -> str:
        ...
        return (
            ...
            f"- {knowledge_authority_guidance('en')}\n\n"
            f"{self._conversation_continuity_contract_en()}"
            "## Work Mode Contract\n"
    ...
    def _trae_rules(self) -> str:
        return (
            ...
            "- Your first reply must report the actual phase: the work-mode entry for new work, or the recorded SESSION_BRIEF phase for resume; do not restart pending work.\n"
            "- Your first reply must explicitly promise the sequence: research -> three core documents -> wait for user confirmation -> Spec/tasks -> applicable frontend runtime verification -> user preview confirmation (standard UI) -> backend/tests/delivery.\n"
            ...
            "- If the user requests architecture changes, first update `output/*-architecture.md`, then realign tasks and implementation before continuing.\n"
            "- If the user requests quality or security remediation, first fix the issues, rerun the quality gate, and refresh any delivery evidence the reports ask for before continuing.\n"
            "- For standard UI work, verify frontend runtime and wait for user preview confirmation before backend work; preserve the non-UI path.\n"
...
- 新流程按工作模式进入；恢复已有流程先读 SESSION_BRIEF 并报告原阶段，不得重置为 research
- 三份核心文档完成后，必须暂停等待用户确认
- 未经用户确认，不得创建 `.super-dev/changes/*` 或开始编码
- **前端先行**：标准 UI 工作先完成可演示前端并等待用户预览确认，再实现后端；无 UI 沿用原不适用处理
