...
"""
    ...
    def _render_spec_template(self, *, title: str, description: str) -> str:
        ...
        return (
            ...
            "## Summary\n"
            f"{summary}\n\n"
            "以下为待填写结构，不是已确认需求或执行证据。沿用原需求编号与权威文档；"
            "仅保留本次适用项，未知先定位，不虚构路径、接口、版本或目标数字。\n\n"
            "## ADDED Requirements\n\n"
            ...
            "- 性能/可靠性/安全：引用本项目已确认且适用的目标与约束\n\n"
            "## Execution Context\n"
            "引用已有设计与词表，不重复维护定义；只填写已核实的影响面。\n\n"
            "| 文件/模块或接口 | 当前定义与版本依据 | 本次变化及消费者 | 已知风险/待核对项 |\n"
            "| --- | --- | --- | --- |\n\n"
            ...
            "- 明确不在本次变更范围的内容\n\n"
            "## Verification\n"
            "- 根据实际影响写出环境、输入、命令或人工步骤、预期输出和未覆盖范围。\n"
            "- 复用现有测试；覆盖适用的成功/失败场景及受影响旧行为。纯文字改动不虚构全栈 E2E。\n"
            "- 实际结果另附代码版本、退出结果和原始证据；计划与空表不表示通过。\n"
        )
    ...
    def _render_plan_template(self, *, title: str, description: str) -> str:
        ...
        return (
            ...
            f"{summary}\n\n"
            "## Architecture Impact\n"
            "- 引用原架构与规格中的实际文件/接口，不另建需求或词表\n"
            "- 核对数据流、调用方与共享状态，记录适用的兼容性影响\n\n"
            "## Risks & Mitigations\n"
            "- 风险1: <描述> -> 缓解: <策略>\n"
            "- 风险2: <描述> -> 缓解: <策略>\n\n"
            "## Rollout Strategy\n"
            "- 按交付形态说明适用的发布与恢复方式，不强制灰度或云部署\n"
            "- 核对恢复授权、目标版本与数据兼容；没有实跑证据不能宣布恢复完成\n"
        )
