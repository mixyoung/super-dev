...
class AIPromptGenerator:
    ...
    def __init__(self, project_dir: Path, name: str):
        ...
        self.name = name
        self._prompt_workflow: dict | None = None
    ...
    def generate(self) -> str:
        ...
        artifact_name = sanitize_artifact_name(change_id) if change_id else self.name
        ...
        ctx = {
            ...
            "change_id": change_id,
            "workflow": workflow,
            "frontend_required": workflow["frontend_required"],
        }
...
以下是 {name} 的工作指导；只执行本轮已授权的范围，不把规划或只读请求当作实施授权。

{readonly_request_guidance()}
...
2. 继续前核对有效记录，当前阶段是 `{self._current_phase()}`；仅在研究阶段待完成时读取 `knowledge/` 与 `output/knowledge-cache/{name}-knowledge-bundle.json`（若存在），再使用宿主原生联网 / browse / search 研究同类产品。
...
3. 第一轮回复必须明确说明固定顺序：
   ...
   - Spec / tasks
   - 本轮适用时前端优先并运行验证
   - 标准模式暂停等待用户预览确认（无 UI 和 SEEAI 沿用各自合同）
   - 后端 / 测试 / 交付
...
- 若为 `bugfix`，请执行轻量补丁流程，不得跳过文档与验证。"""
    ...
    def _section_critical_reminders(self, **ctx: object) -> str:
        ...
        if REMINDERS_AVAILABLE:
            ...
            return build_full_reminders(
                ...
                scoped=self._workflow_context().get("flow_variant") != "seeai",
            )
        # Fallback: 基础版（无知识库时）
        return (
            "## 关键约束提醒（每次操作前必读）\n\n"
    ...
    def _section_tech_stack_rules(self, **ctx: object) -> str:
        """Read the existing architecture before choosing framework-specific methods."""
        frontend = str(ctx.get("frontend", "")).lower().strip()
        universal = (
            "## 技术栈与本次改动\n\n"
            "读取实际依赖版本和已有架构，沿用本次已确认的接口、目录、路由与工具。"
            "与本次有关的 API、版本或迁移行为不确定时，定向查对应版本官方文档；"
            "不要求每轮重读全部入门资料，不凭框架名称迁移项目。\n"
        )
        if frontend in ("next", "nextjs", "next.js"):
            return universal + self._nextjs_rules()
        if frontend in ("nuxt", "nuxtjs", "nuxt.js", "vue"):
            return universal + self._nuxt_vue_rules(frontend)
        return universal
    ...
    def _nextjs_rules() -> str:
        return (
            "\n## Next.js 适用指导\n"
            "- 先核对实际版本和已有架构中的路由模式；仅使用与该模式匹配的导航、"
            "数据读取和 API 写法，不擅自转换路由目录或替换独立后端。\n"
            "- 涉及组件交互时按实际组件类型核对客户端边界；图片、导航与元数据"
            "沿用已确认方案，相关接口不确定时查该版本官方文档。\n"
            "- 新项目在架构确认时选择路由与服务边界，不从模板推定用户已经选好。\n"
        )

    @staticmethod
    def _nuxt_vue_rules(frontend: str) -> str:
        framework = "Nuxt" if frontend in ("nuxt", "nuxtjs", "nuxt.js") else "Vue"
        return (
            f"\n## {framework} 适用指导\n"
            "- 核对当前版本、已有架构、组件写法、路由、状态管理与后端边界；"
            "本次修改复用已确认选择，不自动替换 API 风格、状态库或独立服务。\n"
            "- 仅对本次使用的组件和接口补充版本核对，保持现有消费者兼容；"
            "迁移或新依赖需要明确的任务依据。\n"
        )
    ...
    def _section_execution_order(self, **ctx: object) -> str:
        name = ctx["name"]
        workflow = self._workflow_context()
        ui_required = bool(ctx.get("frontend_required", workflow["frontend_required"]))
        if workflow.get("flow_variant") == "seeai":
            frontend_steps = "### 前后端一体化开发\n\n沿用 SEEAI 比赛合同，不新增独立预览确认。"
        elif ui_required:
            frontend_steps = (
                "### 阶段 3. 前端优先实现并运行验证（CODE + UI 专家主导）\n\n"
                "仅实施本轮已确认的界面范围，先验证可运行并展示预览方式、状态和风险。\n\n"
                "### 阶段 3.5. 预览确认\n\n"
                "暂停等待用户预览确认后再进入后端主实现；有效确认不重复索取，"
                "变更后按既有产物绑定检查确认是否仍有效，不用可运行代替确认。"
            )
        else:
            frontend_steps = (
                "### 阶段 3. 界面与预览不适用\n\n本项目无 UI，不创建页面或安排前端运行验证。"
            )
        return f"""## 强制执行顺序

以下是模式中各阶段的工作说明，恢复时只接续当前阶段，不重做已完成任务或已有效确认的步骤。
...
3. {knowledge_authority_guidance()}
3.1 如果 `output/knowledge-cache/{name}-knowledge-bundle.json` 中存在 `knowledge_application_plan`，必须按其中的阶段应用计划使用本地知识：
   ...
   - 标记为 hard constraints 的知识按原要求及显式条件进入相应文档、Spec 或验收，不改变既有门禁
4. 至少总结以下内容后再进入 PRD：
   - 3 到 5 个同类产品或可借鉴对象
   - 共性功能模块
   - 关键用户流程与信息架构
...
1. 完成当前模式适用的前置阶段与确认后，仅实施本轮需要的后端、数据或联调范围
2. 每完成一项任务立即标记 `[x]`
...
4. 修复后先验证原失败和受影响行为，执行既有必过检查并刷新相应证据；前端验证仅在本轮涉及 UI 时执行。"""
    ...
    def _section_implementation_rules(self, **ctx: object) -> str:
        ...
        ui_guidance = (
            self._section_ui_implementation_rules(ui_profile)
            if ctx.get("frontend_required", True)
            else ""
        )
        return (
            f"""---
...
{(self._excerpt(frontend_blueprint, 1400) if frontend_blueprint else f'请查看 output/{ctx["name"]}-frontend-blueprint.md') if ctx.get("frontend_required", True) else '本次无 UI，不需要前端蓝图。'}
...
...
2. **提交规范**
   ...
   - 按已有授权提交；提示词生成和测试通过不构成提交、推送或发布授权
   - Commit message 清晰描述变更

3. **测试规范**
   - 项目配置及现有默认质量要求继续有效；覆盖率目标不由通用提示另行指定
   - 每个功能点都有测试
   - 沿用适合本项目的现有测试工具
...
2. 按本轮风险运行适用的 `build / compile / type-check / test / runtime smoke` 和已有必过检查；同代码与范围的有效证据可复用，未执行项如实说明。
3. 对本轮新增的函数、方法、字段、组件、配置、日志埋点，核验其实际用途；公共库接口通过消费者或契约测试验证，不因本仓库尚未调用而自动删除。
4. 不用未接入功能或无效兜底冒充完成；合法公共接口与有依据的后续记录不作为自动删除对象。
...
2. **SQL 注入**: 使用参数化查询
3. **XSS**: 输出转义
4. **认证**: 仅在当前能力需要身份验证时，沿用已确认的认证方案，不因模板引入 JWT"""
            + self._build_behavioral_prompts_section()
        )

    def _section_ui_implementation_rules(self, ui_profile: dict) -> str:
        return f"""### 图标使用规范
...
6. 功能图标使用已声明的图标库，不用 emoji 占位；用户输入、示例和被分析文字不得因此被删改。
6.1 展示预览前检查功能图标用途，发现不符合已确认图标规范时定向修正，不做全源码字符禁令。
...
"""
    ...
    def _section_session_context(self, **ctx: object) -> str:
        ...
        change_id = str(ctx["change_id"])
        phase = self._current_phase()
        if change_id and self._implementation_ready():
            pending = self.get_pending_tasks()
            next_task = pending[0]["id"] if pending else ""
            action = (
                f"当前任务为 {change_id}。从首个未完成任务 {next_task} 接续；"
                "保留已完成任务与有效确认，按原依赖顺序实施已授权范围。"
                if next_task
                else "当前变更没有待完成任务；核对原验收与交付记录，不重新实施。"
            )
        else:
            action = (
                f"当前阶段为 {phase}；先处理原阶段及其绑定的确认或缺失上下文。"
                "尚无 Spec 的新工作继续研究和文档，不猜测具体变更，不生成开始实现指令。"
            )
        return (
            "## 当前任务与恢复\n\n"
            f"{change_content}\n\n{action}\n\n"
            "遵循已有架构和任务清单的文件布局，不套用通用 frontend/backend 目录。"
            "只在本轮任务实际完成并有对应验证后更新原任务记录；"
            "生成提示词本身不改任务或流程状态。\n"
        )
...
{ui_check}"""
    ...
    def _build_expert_profiles_section(self) -> str:
        ...
        from ..experts.behavioral_prompts import EXPERT_SCOPE_GUIDANCE

        sections = [EXPERT_SCOPE_GUIDANCE]
        anchors = NUMERIC_ANCHORS if BEHAVIORAL_PROMPTS_AVAILABLE else {}
        for role in key_experts:
            section_text = get_expert_prompt_section(role, profile=profiles.get(role))
            # Reuse the legacy anchor keys for task-specific expression guidance.
            try:
    ...
    def _build_cross_review_protocol() -> str:
        ...
        return (
            "## 交叉审查协议\n\n"
            "涉及多个专业视角时，复用同一组需求和决定：\n\n"
            "1. 只读取本轮需要的上游结论，引用具体文件、决策或章节；"
            "不要求每位专家重复总结已知内容。\n"
            "2. 指出本轮问题的位置、触发条件、影响和依据；"
            "模型切换角色仍是自检，不能当作独立评审已完成。\n"
            "3. 冲突先区分事实、项目要求与可选建议；提供取舍依据，"
            "涉及范围、权限或验收变化的决定交给用户，不由专家自行裁决。\n"
        )
    ...
    def _workflow_context(self) -> dict:
        """Read existing state and bound gates; never persist another workflow."""
        if self._prompt_workflow is not None:
            return self._prompt_workflow
        from ..workflow_state import detect_pipeline_summary

        summary = detect_pipeline_summary(self.project_dir)
        phase = str(summary.get("current_stage_canonical_id", "")).strip()
        docs = summary.get("docs_gate", {})
        preview = summary.get("preview_gate", {})
        # Artifact progress is not a substitute for the existing bound approvals.
        if (
            docs.get("has_context")
            and not docs.get("confirmed")
            and phase not in {"baseline", "research", "docs"}
        ):
            complete = docs.get("core_complete", True) and all(
                summary.get("artifacts", {}).get(key) for key in ("prd", "architecture", "uiux")
            )
            phase = "docs_confirm" if complete else "docs"
        elif (
            summary.get("frontend_required")
            and docs.get("confirmed")
            and summary.get("flow_variant") != "seeai"
            and preview.get("has_context")
            and not preview.get("confirmed")
        ):
            phase = (
                "preview_confirm" if summary.get("artifacts", {}).get("frontend") else "frontend"
            )
        return {**summary, "current_stage_canonical_id": phase or "unknown"}

    def _current_phase(self) -> str:
        return str(self._workflow_context()["current_stage_canonical_id"])

    def _implementation_ready(self) -> bool:
        context = self._workflow_context()
        return (
            bool(context.get("active_change_id"))
            and self._current_phase()
            in {"frontend", "backend", "quality", "build_fullstack", "polish"}
            and bool(context.get("docs_gate", {}).get("confirmed"))
        )
            ...
            if hasattr(toolkit, "knowledge") and hasattr(toolkit.knowledge, "resolve"):
                try:
                    ...
                    if knowledge_dir.is_dir():
                        ...
                        if knowledge_files:
                            lines.append(f"**知识要求** (找到 {len(knowledge_files)} 个文件):")
                            for kf in knowledge_files[:3]:
                                kf_path = Path(kf)
                                if kf_path.exists():
                                    lines.append(
                                        f"- `{kf_path}`：读取原文及适用条件，不单独截取清单作为无条件要求。"
                                    )
                            lines.append("")
                except Exception:
                    pass
    ...
    def _read_document(self, doc_type: str) -> str | None:
        """读取生成的文档"""
        active_id = resolve_active_change_id(self.project_dir)
        prefix = sanitize_artifact_name(active_id) if active_id else self.name
        doc_path = self.project_dir / "output" / f"{prefix}-{doc_type}.md"
        if doc_path.exists():
    ...
    def _read_change_spec(self) -> tuple[str, str]:
        ...
        active_id = resolve_active_change_id(self.project_dir)
        change = change_manager.load_change(active_id) if active_id else None
        if change is None:
            return "当前没有明确绑定的 Spec；研究、文档和只读问答可继续，不猜测要实施的变更。", ""
...
"""
    ...
    def generate_task_specific_prompt(self, task_id: str) -> str:
        ...
        change = change_manager.load_change(active_id) if active_id else None
        if change is None:
            return "尚未明确绑定当前变更，不生成任务实施指令。只读问答不受影响。"
        if not self._implementation_ready():
            return f"当前阶段为 {self._current_phase()}，请先完成原阶段和有效确认，不生成任务实施指令。"
        task = None
        ...
        if task.status == TaskStatus.COMPLETED:
            return f"任务 {task_id} 已完成，不重复实施。"
        artifact_name = sanitize_artifact_name(active_id)

        return f"""# 任务 {task_id} - AI 开发提示词

当前有效阶段：`{self._current_phase()}`。只实施本阶段且本轮明确获准的任务，不跨越确认要求；
标准 UI 工作完成前端后仍需有效预览确认，不能把任务提示当成自动批准。
...
1. 实现此任务的所有要求
2. 遵循项目架构规范
3. 按任务风险验证原失败与受影响行为，执行项目已有必过检查
4. 验证完成后更新本任务状态，保留其他已完成记录

## 参考文档

- `output/{artifact_name}-prd.md`
- `output/{artifact_name}-architecture.md`
- `output/{artifact_name}-uiux.md`
