...
- `workflow-state.json` 成为唯一当前流程状态，并新增修订冲突、跨平台锁、原子提交、历史快照和恢复基础。
- `SESSION_BRIEF.md` 改为根据最新状态自动生成并携带工作项与状态修订。
- `pipeline-state.json` 只允许一次迁移，成功后删除；引擎和专家阶段检查统一读取当前状态与阶段记录。
- 新增候选绑定的证据信封，区分代码、输入和制品摘要；代码变化后旧验证不再支持当前通过。
- 新增正式发布观察记录，只更新 `released`，不推导部署或运营成功。
- Fresh Verification 改用受控临时文件收集输出，不再被测试内后台子进程继承的管道拖住；长时运行每分钟显示心跳，并强化正常结束、超时与取消时的子进程清理。
...
- 建立 L0-L4 内容权威、确定性知识信封与状态/工具控制守卫；仓库和外部指令只作为待审数据，不能自行取得权限。
- 收敛阶段专家单一真源与 ExpertProfile v2，记录适用性、输入输出、权限、证据、停止和交接合同；独立评审必须绑定真实会话与候选。
- 为标准流程增加 Spec 前工作项身份、文档摘要绑定和原子状态写入；旧 change、确认与证据不能污染新任务，SEEAI 既有路径保持不变。
- 解耦 `delivery_ready`、`released`、`deployed`、`operating` 四类事实；候选摘要排除用户本地配置与机器级 Git ignore 差异。
- 修正 Feature Checklist 将研究优先级误判为缺口的问题；纯路线图只有关联当前未完成任务时才计作 gap，unknown 不伪装为 covered。

### 修复

- 提示生成绑定明确的当前变更，恢复未完成任务，不再按最新文件或固定阶段猜测执行上下文。
- 对齐只读请求、标准预览确认与无界面项目的适用边界，保留既有确认和质量要求。
- 对齐知识、专家、宿主及框架提示；无显式适用条件的知识要求继续默认强制，明确条件按条件执行。
- 修复常规 QA 被验证变体覆盖、专家自定义设定未进入提示及引号解析问题，补充相关回归。

### 保留边界

- 不增加主流程、专家或知识自循环，不改变默认质量阈值。
- SEEAI 比赛内容保持不变，仅同步版本元数据；不更新用户全局安装。
- 本版本仅通过 `mixyoung/super-dev` GitHub Release 发布 wheel、sdist 与校验和，不上传 PyPI，也不执行部署。详见 [2.5.1](docs/releases/2.5.1.md)。

## [2.5.0] - 2026-09-07

本 fork 的 GitHub-only 发行版；保留宿主执行、Super Dev 治理的使用方式。完整说明见 [2.5.0](docs/releases/2.5.0.md)。

### 新增与完善

- 结构化范围声明、只读阶段影子台账与对照；不授予影子台账流程控制权。
- 最小扩展机制与完成前验证：来源锁、权限、执行器、证据、所有权和路径检查。
- 择优吸收 Forge、需求审查、Grill 与 UmaDev 的软内容；固定贡献和演进边界，不引入独立 UmaDev 程序或新必经阶段。
- Windows 测试隔离与真实必过 CI，项目必需/用户可选的宿主就绪口径。

### 修复

- 类型与外部输入边界、缓存类型隔离、UI 契约安全文件名、中文编码、跨平台路径、日志释放与 Git Bash 选择。
- 规则解析内容缓存与实例隔离，知识索引绑定目标项目；性能脚本失败正确返回非零。
- 七个依赖的已知漏洞版本下限，修正安全 CI 的错误调用和吞错。
- 补齐发行包遗漏的既有规则 YAML、设计数据、文档模板及 Web 静态入口。

### 发布边界

- 不上传原作者 PyPI 包；安装/升级使用本 fork 的 Git 标签或 Release wheel。
- 标准流程、确认权与质量阈值不变；冷启动和热初始化指标分别报告。
- 可选宿主插件包不作为本次独立发行制品，旧开发任务的应用验收状态不冒充包发布结果。
