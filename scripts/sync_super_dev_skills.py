...
def rendered_skill(host: str) -> str:
    from super_dev.skills.skill_template import SkillTemplate

    return SkillTemplate.for_builtin("super-dev", host).render(host)
...
def collect_mismatches(user_codex_path: Path | None = None) -> list[Path]:
    ...
    for host, paths in TRACKED_SURFACES.items():
        ...
        mismatches.extend(
            path for path in paths if not path.exists() or path.read_text(encoding="utf-8") != expected
        )
    if user_codex_path is not None:
        expected = rendered_skill("codex")
        if (
            not user_codex_path.exists()
            or user_codex_path.read_text(encoding="utf-8") != expected
        ):
