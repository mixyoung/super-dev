...
6. 当前先完成已有可靠性收尾，再落实小范围方法与知识升级；方法指导不以 Phase 2B 程序扩展晋级为前置。
...
优先改进现有 `knowledge/development/02-playbooks/debugging-playbook.md` 与 RCA 指导，不等待 Phase 2B 程序扩展晋级，也不新建排错服务。
...
TDD 是当前任务的实现策略，不以评审扩展或 Phase 2B 晋级为前置，不设为全局强制。视觉探索、低风险配置和暂不可自动验证的集成使用适合任务的替代证据；不得为补做 TDD 删除已接受实现。
...
| Phase 2B：程序化排错候选 | 新增自动执行能力的评估入口 | 净收益和单独限定范围仍必要；不约束既有排错手册的改进 |
...
上表中的后续程序能力和扩大试用均为候选工作，不能由宿主依据编号自动推进。本次不修改 Phase 2B 指标计算或真实候选标签；任何效果结论都须绑定实际观测。
